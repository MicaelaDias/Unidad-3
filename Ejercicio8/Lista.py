...
class Lista:
    ...
    #IDirector
    def modificarBasico(self,dni,sueldo):
        aux=self.__comienzo
        bandera=True
    ...
    def modificarPorcentajeporcargo(self,dni,porentaje):
        aux=self.__comienzo
        bandera=True
        res=None
        while aux!=None and bandera:
            if aux.getDato().getCuil()==dni:
                if isinstance(aux.getDato(),Docente):
                    aux.getDato().setPorcentaje(porentaje)
                else:
                    res=-1
                bandera=False
            else:
        ...
        if res!=-1:
            if bandera:
                ...
                print('Se modifico el porcentaje')
        else:
            print('NO ES UN DOCENTE')
